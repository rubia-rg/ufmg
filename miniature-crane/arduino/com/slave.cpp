...
int16_t rot_angle = 0;
int16_t ica_angle = INITIAL_THETA;
...
void setup()
{
  ...
  // Init Motor
  pinMode(MOTOR_ROT_PIN, OUTPUT);
  pinMode(DIRECTION_ROT_PIN, OUTPUT);

  // Init Motor
  pinMode(MOTOR_ICA_PIN, OUTPUT);
  pinMode(DIRECTION_ICA_PIN, OUTPUT);

  // Init Encoder
  pinMode(ENCODER_ROT_PIN, INPUT);
  pinMode(ENCODER_ICA_PIN, INPUT);
...
void stop()
{
  analogWrite(MOTOR_ROT_PIN, 0);
  digitalWrite(DIRECTION_ROT_PIN, LOW);

  analogWrite(MOTOR_ICA_PIN, 0);
  digitalWrite(DIRECTION_ICA_PIN, LOW);
}
